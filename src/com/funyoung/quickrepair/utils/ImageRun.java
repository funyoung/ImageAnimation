...
import com.funyoung.quickrepair.utils.thread.QueuedThreadPool;
    ...
    public boolean setImageView(View view) {
        ...
        if (cacheBmp != null && cacheBmp.getWidth() > 0) {
            ...
            if(!TextUtils.isEmpty(scheme) && scheme.equalsIgnoreCase("file")) {
                ...
                localpath = QiupuHelper.isImageExistInPhone(view.getContext(), url, addHostAndPath);
            }
            if (localpath != null) {
                        ...
                        public void run() {
                            try {
                                ...
                                if (localpath.startsWith("/assets/")) {
                                    ...
                                    try {
                                        final String assetsPath = localpath.substring("/assets/".length());
                                        inputStream = imgView.getContext().getAssets().open(assetsPath);
                                        ...
                                        Log.d(TAG, "tmp.getWidth()=" + tmp.getWidth());
                                        Log.d(TAG, "tmp.getHeight()=" + tmp.getHeight());
                ...
                pHandler.post(new Runnable() {
                    public void run() {
                        // checkAndSeetDefault();
                        if (noimage == false) {
                            final int res = R.drawable.avatar_boy;
                            /*
    ...
    public void run() {
        ...
        if (imgView != null) {
            if (fromlocal == false && isSavedMode == false) {
                bmp = QiupuHelper.getImageFromURL(imgView.getContext(), url,
                ...
                Log.d(TAG, "bmp.getHeight()=" + bmp.getHeight());
            }
            ...
            if (bmp == null) {
                ...
                if (v != null && v.equals(url)) {
                    pHandler.post(new Runnable() {
                        public void run() {
                            if (noimage == false) {
                                setImageBmp(R.drawable.avatar_boy);
                            } else {
                                dispose();
                            }
                        }
